                ...
                <div class="cont-content">
                    <ul >
                        <li class="list" v-for="(item,index) in listData" :key="index">
                            <p><span v-if="item.PayInto>0" style="color:#009788;">收入</span><span v-if="item.PayOut>0" style="color:#DF8D15;">支出</span><em v-if="item.PayInto>0" style="color:#009788;">+{{item.PayInto}}</em><em v-if="item.PayOut>0" style="color:#DF8D15;">-{{item.PayOut}}</em></p>
                            <p><span>{{item.JoinDate}}</span><em>{{item.CurrentMoney}}</em></p>
                            <p v-if="item.SourceType == 2">来源单行:<span>{{item.SourceNo}}</span></p>
                            <p v-else>来源:<span>{{item.SourceInfo}}</span></p>
                        ...
                        <div class="list-foot" :class="listData.length == 0 ? 'no' : ''">
                            ...
                            <div v-else class="notData">
                                <p><img src="../../assets/images/noBill.png" alt="" style="width:3rem;vertical-center:bottom;"></p>
                                <p>当前没有数据</p>
                            </div>
                        </div>
                    </ul>
...
export default {
    ...
    methods:{
        ...
        load(id,bool){
            ...
            if(that.isUserId == 0){
                wxget('User/GetBalanceSlip/'+that.selRow.PageSize+'/'+that.selRow.CurPage+'/'+that.selRow.typeId+'/'+that.selRow.plocyId+'/'+that.selRow.loadId).then(res =>{
                    that.$Spin.hide();
                    if(res.code == 200){
                ...
                wxget('User/GetBalanceSlipByUserId/'+that.selRow.PageSize+'/'+that.selRow.CurPage+'/'+that.isUserId+'/'+that.selRow.typeId+'/'+that.selRow.plocyId+'/'+that.selRow.loadId).then(res =>{
                    that.$Spin.hide();
                    if(res.code == 200){
                        that.listData.push(...res.data);
                        that.selRow.allTotal = res.total;
    ...
    mounted(){
        let that = this;
        that.isUserId = that.$route.params.id;
        // if(that.isUserId == 0){
        ...
        that.statusData = [{status:0,text:'全部'},{status:1,text:'收入'},{status:2,text:'支出'}];
