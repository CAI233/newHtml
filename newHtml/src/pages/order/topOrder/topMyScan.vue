<template>
    <div id="container" class="topMyScan">
        <div class="content">
...
export default {
                                ...
                                h('i', {style:{padding:'0.1333rem'},
                                    on: {
                                        click: () => {
                                            ...
                                            addCookie('topMyScanData',JSON.stringify(that.scanData),1);
                                        }
                                    }
                                }, '删除')
                            ]);
    ...
    methods:{
        load(){//加载缓存
            let that = this;
            that.scanData = getCookie('topMyScanData') ? JSON.parse(getCookie('topMyScanData')) : [];
            that.formItem.num = that.scanData.length;
        ...
        sureClick(){//条码输入
            ...
            wxpost('User/CheckScanCodeForUsedself',{ScanCode:that.formItem.scan}).then(res =>{
                that.$Spin.hide();
                if(res.code == 200){
                    console.log(res);
        ...
        sureSubmit(){//确认提交
            ...
            wxpost('User/InsertGoodsByScanCodeByUsedself',{scanCodeJson:JSON.stringify(that.scanData)}).then(res =>{
                that.$Spin.hide();
                if(res.code == 200){
...
<style>
    /* .topMyScan .content .cont_cont .cont_d .ivu-table-wrapper{
        border:none;
    } */
    .topMyScan .content .cont_cont .cont_d .ivu-table-wrapper .ivu-table{
        background:none;
    }
    .topMyScan .content .cont_cont .cont_d .ivu-table-wrapper .ivu-table th{
        text-align:center;
        color:#727171;
        background:#fff;
        height:1rem;
    }
    .topMyScan .content .cont_cont .cont_d .ivu-table-wrapper .ivu-table td{
        height:1rem;
    }
    .topMyScan .content .cont_cont .cont_d .ivu-table-wrapper .ivu-table .ivu-table-cell{
        padding-left:0;
        padding-right:0;
        text-align:center;
        font-size:0.32rem;
