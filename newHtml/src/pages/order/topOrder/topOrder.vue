<template>
    <div id="container" class="topOrder">
        <div class="content">
            <div class="title">
                <div class="title_a">
...
export default {
    ...
    methods:{
        ...
        load(bool){//加载数据
            ...
            wxpost('User/GetToPurchaseOrder',that.selRow).then(res =>{
                that.$Spin.hide();
                if(res.code == 200){
                    console.log(res);
        ...
        getCount(){//获取各种订单数量
            ...
            wxpost('User/GetTopOrdStaCount',statusCount).then(res =>{
                if(res.code == 200){
        ...
        goDetails(no){//订单详情
            let that = this;
            that.$router.push({name:'topOrderDetails',params:{OrderNo:no}});
        },
        Pay(no,price){//去付款
            let that = this;
            let payWord = '';
                                ...
                                h('Button', {'style':{boxShadow:'none',background: '#009788',borderColor:'#009788'},props: {type: 'primary'},on: {click:()=>{
                                    ...
                                    wxpost('API/PostEn',{Content:payWord}).then(res =>{//支付密码加密
                                        if(res.code == 200){
                                            ...
                                            wxpost('User/SetToPurchaseOrderPay',payParam).then(re =>{//支付接口
                                                that.$Spin.hide();
                                                if(re.code == 200){
                                                    that.$Modal.remove();
                ...
                render: (h) => {
                    return h('div',{class:'model-cont'},[
                        ...
                        h('div',{class:'model-foot','style':{textAlign:'right',marginTop:'0.2667rem'}},[
                            ...
                            h('Button', {'style':{boxShadow:'none',background: '#009788',borderColor:'#009788'},props: {type: 'primary'},on: {click:()=>{
                                isLoading(that,'取消中···')
                                wxpost('User/SetToPurchaseOrderCancel',{OrderNo:no}).then(res =>{
                                    that.$Spin.hide();
                                    if(res.code == 200){
        ...
        confirmErp(no,num){//erp确认订单  没有更改
            let that = this;
            that.$Modal.confirm({
                scrollable:false,
                render: (h) => {
...
<style>
    .topOrder .content .title .title_a .ivu-dropdown .ivu-dropdown-rel{
        padding:0.1333rem 0.2667rem;
        background:#DCDDDD;
    }
    .topOrder .content .title .title_a .ivu-dropdown .ivu-select-dropdown{
        left:0.4rem;
        width:-moz-calc(100% - 0.8rem);
        width:-webkit-calc(100% - 0.8rem);
        width:calc(100% - 0.8rem);
    }
    .topOrder .ivu-dropdown-item-selected,.topOrder .ivu-dropdown-item-selected:hover{
        background:none;
        color:#009788;
    }
    .topOrder .content .cont>ul>.active a{
        color:#009788 !important;
    }
    .topOrder .ivu-btn,.topOrder .ivu-btn:hover{
        border-color:#727171;
        color:#727171;
    }
    .topOrder .ivu-btn.nopay,.topOrder .ivu-btn.nopay:hover{
        border-color:#009788;
        /* color:#009788; */
    }
    ...
    .topOrder .ivu-btn.notime,.topOrder .ivu-btn.notime:hover{
        border-color:#DF8D15;
        color:#DF8D15;
    }
</style>
