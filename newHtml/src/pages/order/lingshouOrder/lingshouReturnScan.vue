<template>
    <div id="container" class="lingshouReturnScan">
        <div class="content">
            ...
            <div class="cont_cont">
                <ul class="cont_u">
                    ...
                    <li>
                        <span>输入条码:</span><input type="text" v-model="formItem.scan" placeholder="输入条码"/><button type="button" @click="sureClick">输入</button>  
                    </li>
                    <li>
                        <span>输入退货人姓名:</span><input type="text" v-model="formItem.BuyName" placeholder="输入退货人姓名"/>  
                    </li>
                </ul>
                <div class="cont_d">
...
export default {
                                ...
                                h('i', {style:{padding:'0.1333rem'},
                                    on: {
                                        click: () => {
                                            ...
                                            addCookie('lingshouReturnScanData',JSON.stringify(that.scanData),1);
                                        }
                                    }
                                }, '删除')
                            ]);
    ...
    methods:{
        sureClick(){//条码输入
            ...
            if(!that.formItem.BuyName || that.formItem.BuyName == ''){
                that.$Message.warning('请输入退货人姓名！');
                return false;
            }
            isLoading(that,'条码确认中···');
            wxpost('User/CheckScanCodeForRetailReturn',{ScanCode:that.formItem.scan,BuyName:that.formItem.BuyName}).then(res =>{
                that.$Spin.hide();
                if(res.code == 200){
                    console.log(res);
        ...
        sureSubmit(){//确认提交
            ...
            wxpost('User/InsertGoodsByScanCodeByRetailReturn',{scanCodeJson:JSON.stringify(that.scanData),BuyName:that.formItem.BuyName}).then(res =>{
                that.$Spin.hide();
                if(res.code == 200){
        ...
        sureCancel(){//清除
            ...
            that.formItem.num = 0;
            that.formItem.scan = '';
            that.formItem.BuyName = '';
            deleteCookie('lingshouReturnScanData');
        },
        sureScan(){//扫码
            console.log(111);
            // alert(1111111);
    ...
    mounted(){
        let that = this;
        that.scanData = getCookie('lingshouReturnScanData') ? JSON.parse(getCookie('lingshouReturnScanData')) : [];
        that.formItem.num = that.scanData.length;
...
<style>
    .lingshouReturnScan .content .cont_cont .cont_d .ivu-table-wrapper .ivu-table{
        background:none;
    }
    .lingshouReturnScan .content .cont_cont .cont_d .ivu-table-wrapper .ivu-table th{
        text-align:center;
        color:#727171;
        background:#fff;
        height:1rem;
    }
    .lingshouReturnScan .content .cont_cont .cont_d .ivu-table-wrapper .ivu-table td{
        height:1rem;
    }
    .lingshouReturnScan .content .cont_cont .cont_d .ivu-table-wrapper .ivu-table .ivu-table-cell{
        padding-left:0;
        padding-right:0;
        text-align:center;
        font-size:0.32rem;
    }
    .lingshouReturnScan .content .cont_cont .cont_u li .ivu-dropdown .ivu-select-dropdown{
        left: auto !important;
        right: 0.4rem;
        max-height: 4rem;
