<template>
    <div id="container" class="lingshouScan">
        <div class="content">
            ...
            <div class="cont_cont">
                <ul class="cont_u">
                    ...
                    <li>
                        <span>输入购买人姓名:</span><input type="text" v-model="formItem.BuyName" placeholder="输入购买人姓名"/>  
                    </li>
                </ul>
                <div class="cont_d">
...
export default {
                                ...
                                h('i', {style:{padding:'0.1333rem'},
                                    on: {
                                        click: () => {
                                            ...
                                            addCookie('lingshouScanData',JSON.stringify(that.scanData),1);
                                        }
                                    }
                                }, '删除')
                            ]);
    ...
    methods:{
        sureClick(){//条码输入
            ...
            wxpost('User/CheckScanCodeForRetail',{ScanCode:that.formItem.scan}).then(res =>{
                that.$Spin.hide();
                if(res.code == 200){
                    console.log(res);
        ...
        sureSubmit(){//确认提交
            ...
            wxpost('User/InsertGoodsByScanCodeByRetail',{scanCodeJson:JSON.stringify(that.scanData),BuyName:that.formItem.BuyName}).then(res =>{
                that.$Spin.hide();
                if(res.code == 200){
    ...
    mounted(){
        let that = this;
        that.scanData = getCookie('lingshouScanData') ? JSON.parse(getCookie('lingshouScanData')) : [];
        that.formItem.num = that.scanData.length;
...
<style>
    .lingshouScan .content .cont_cont .cont_d .ivu-table-wrapper .ivu-table{
        background:none;
    }
    .lingshouScan .content .cont_cont .cont_d .ivu-table-wrapper .ivu-table th{
        text-align:center;
        color:#727171;
        background:#fff;
        height:1rem;
    }
    .lingshouScan .content .cont_cont .cont_d .ivu-table-wrapper .ivu-table td{
        height:1rem;
    }
    .lingshouScan .content .cont_cont .cont_d .ivu-table-wrapper .ivu-table .ivu-table-cell{
        padding-left:0;
        padding-right:0;
        text-align:center;
        font-size:0.32rem;
    }
    .lingshouScan .content .cont_cont .cont_u li .ivu-dropdown .ivu-select-dropdown{
        left: auto !important;
        right: 0.4rem;
        max-height: 4rem;
