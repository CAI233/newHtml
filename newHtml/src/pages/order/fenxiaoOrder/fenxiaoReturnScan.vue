<template>
    <div id="container" class="fenxiaoReturnScan">
        <div class="content">
...
export default {
                                ...
                                h('i', {style:{padding:'0.1333rem'},
                                    on: {
                                        click: () => {
                                            ...
                                            addCookie('fenxiaoReturnScanData',JSON.stringify(that.scanData),1);
                                        }
                                    }
                                }, '删除')
                            ]);
    ...
    methods:{
            ...
            wxget('User/GetAllUserRefereeById').then(res =>{
                if(res.code == 200){
                    ...
                    if(that.dealerData.length>0){//默认第一个
                        ...
                        that.formItem.dealerText = '-请选择退货经销商-';
        ...
        sureClick(){//条码输入
            ...
            if(!that.formItem.dealerId || that.formItem.dealerId == ''){
                that.$Message.warning('请选择退货经销商');
                return false;
            }
            isLoading(that,'条码确认中···');
            wxpost('User/CheckScanCodeForUndertakesReturn',{ScanCode:that.formItem.scan,UserCode:that.formItem.dealerId}).then(res =>{
                that.$Spin.hide();
                if(res.code == 200){
                    console.log(res);
        ...
        sureSubmit(){//确认提交
            ...
            wxpost('User/InsertGoodsByScanCodeByUndertakesReturn',{scanCodeJson:JSON.stringify(that.scanData),UserCode:that.formItem.dealerId}).then(res =>{
                that.$Spin.hide();
                if(res.code == 200){
        ...
        sureCancel(){//清除
            ...
            that.formItem.dealerText = '-请选择退货经销商-';
            deleteCookie('fenxiaoReturnScanData');
        },
        sureScan(){//扫码
            console.log(111);
            // alert(1111111);
    ...
    mounted(){
        let that = this;
        that.scanData = getCookie('fenxiaoReturnScanData') ? JSON.parse(getCookie('fenxiaoReturnScanData')) : [];
        that.formItem.num = that.scanData.length;
        that.getDealer();
...
<style>
    .fenxiaoReturnScan .content .cont_cont .cont_d .ivu-table-wrapper .ivu-table{
        background:none;
    }
    .fenxiaoReturnScan .content .cont_cont .cont_d .ivu-table-wrapper .ivu-table th{
        text-align:center;
        color:#727171;
        background:#fff;
        height:1rem;
    }
    .fenxiaoReturnScan .content .cont_cont .cont_d .ivu-table-wrapper .ivu-table td{
        height:1rem;
    }
    .fenxiaoReturnScan .content .cont_cont .cont_d .ivu-table-wrapper .ivu-table .ivu-table-cell{
        padding-left:0;
        padding-right:0;
        text-align:center;
        font-size:0.32rem;
    }
    .fenxiaoReturnScan .content .cont_cont .cont_u li .ivu-dropdown .ivu-select-dropdown{
        left: auto !important;
        right: 0.4rem;
        max-height: 4rem;
