        ...
        <div class="content">
            ...
            <div class="cont_down">
                ...
                <div class="mails-list" v-for="(item,index) in mailsList" :key="index">
                    ...
                    <div class="down_f">
                        <p><span @click="electronPrint(index)">电子打印</span><span @click="epaperyPrint(index)">纸质打印</span></p>
                    </div>
                </div>
...
export default {
    ...
    data() {
        return{
            ...
            remark:'',
            electronList:[],//电子打印-快递列表
            electronTitle:'--请选择快递--',
            electronId:'',
            epaperyList:[],//纸质打印-快递列表
        }
    },
    components: {
    ...
    methods:{
        load(id){//加载订单
            let that = this;
            wxget('User/GetSaleOrderByOrderNo2/'+id).then(res =>{
                if(res.code == 200){
                    console.log(res);
        ...
        deliveryOrder(){//确认发货
            ...
            if(that.goodsIds){//分批发货
                url = 'User/SetPurchaseSendOrderDeliver';
                let postGoods = []
                ...
                url = 'User/SetPurchaseOrderDeliver';
        ...
        submitOrder(url,param){
            let that = this;
            wxpost(url,param).then(res =>{
                if(res.code == 200){
                    let nowScanData = {
                        UserCode:that.orderData.BuyUserId,
                        scanCodeJson:[]
                    };
                    ...
                    wxpost('User/InsertGoodsByScanCodeByUndertakes',nowScanData).then(res =>{
                        if(res.code == 200){
        ...
        electronPrint(index){//电子打印
            let that = this;
            wxget('User/GetUserInterExpressConfig').then(res =>{
                if(res.code == 200){
                    that.electronList = res.data;
                    that.electronList.unshift({
                        F_Id:'',
                        Title:'-请选择快递-'
                    })
                    that.$Modal.confirm({
                        ...
                        render: (h) => {
                            return h('div',{class:'model-cont'},[
                                h('div',{class:'model-title','style':{textAlign:'right',marginBottom:'0.2667rem'}},[
                                    h('span',
                                        {style:{color:'#f90',fontSize:'0.3733rem',background:'#F8F8F8',float:'left'}}, '选择要打印的快递'),
                                    ...
                                    h('p',{style:{fontSize:'0.3733rem',display:'flex'}},[
                                        ...
                                        h('Dropdown',{style:{flex:'1'},props:{trigger:'click'},on:{
                                            ...
                                            h('a',{attrs:{href:'javascript:void(0)'},style:{display:'inherit',textAlign:'right',color:'#009788'}},[
                                                h('span',{style:{float:'left'}},that.electronTitle),
                                                h('Icon',{props:{type: 'ios-arrow-down'}})
                                            ]),
                                            h('DropdownMenu',{slot:'list'},that.electronList.map((item,index) =>{
                                                return h('DropdownItem',{props:{name:item.F_Id,key:index,selected:item.F_Id == that.electronId ? true : false},nativeOn:{'click':() =>{//大坑 //https://www.jianshu.com/p/034afd50ac21 
                                                    that.electronId = item.F_Id;
                                                    that.electronTitle = item.Title;
                                                }}},item.Title)
                                            }))
                                        ])
                                    ])
                                ...
                                h('div',{class:'model-foot','style':{textAlign:'right',marginTop:'0.2667rem'}},[
                                    ...
                                    h('Button', {'style':{boxShadow:'none',background: '#009788',borderColor:'#009788'},props: {type: 'primary'},on: {click:()=>{
                                        if(!that.electronId || that.electronId == ''){
                                            that.$Message.warning('请选择快递');
                                            return false;
                                        }
                                        isLoading(that,'打印中···');
                                        let toPrintParam = {
                                            OrderNo:that.orderNo,
                                            InterExpressId:that.electronId,
                                            GoodsDetailJson:'',
                                            PayType:1
                                        }
                                        wxpost('User/SetSellInterExpressPrint',toPrintParam).then(res =>{
                                            that.$Spin.hide();
                                            that.$Modal.remove();
                                            if(res.code == 200){
