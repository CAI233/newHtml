<template>
    <div id="container" class="fenxiaoOrder">
        <div class="content">
            <div class="title">
                <div class="title_a">
                    ...
                    <ul class="cont_c_cont">
                        <li class="cont_list" v-for="(item,index) in orderData" :key="index">
                            ...
                            <div class="cont_list_u" v-if="item.OrderDetailList.length>0" v-for="(it,num) in item.OrderDetailList" :key="num" @click="goDetails(item.OrderNo)">
                                <div class="list_u_l">
                                    <!-- <img v-if="!it.ImageUrl" src="../../../assets/images/user.png" /> -->
                                    <img v-lazy="imgUrl+it.ImageUrl" :key="it.ImageUrl"  />
                                    <!-- <img :src="imgurl+it.ImageUrl"  /> -->
                                </div>
                                <div class="list_u_c">
                                    <p>{{it.Title}}</p>
                                    <p>规格:{{it.SepcName}}&nbsp;&nbsp;&nbsp;&nbsp;箱规:{{it.UnitNumber}}/{{it.Unit}}</p>
                                    <p><span>¥{{it.Price}}*{{it.Number}}</span>&nbsp;&nbsp;<i class="zu">{{it.IsGroup == true ?'组':''}}</i><i>{{it.SendNo && (item.OrderStatus==33||item.OrderStatus==3)?'(已发货)':''}}</i>&nbsp;&nbsp;<em>小计:{{it.Amount}}</em></p>
                                </div>
                            </div>
                            <Divider />
                            <div class="cont_list_d">
                                <div class="list_d_l">
                                    <img v-if="!item.BuyUserImg" src="../../../assets/images/user.png"  />
                                    <img v-else v-lazy="item.BuyUserImg" :key="item.BuyUserImg" alt="">
                                </div>
                                <div class="list_d_c">
                            ...
                            <div class="cont_list_f">
                                ...
                                <p>
                                    ...
                                    <Button v-if="item.ActivityType==3 && item.OrderStatus==22">已支付</Button>
                                    ...
                                    <Button v-if="(item.OrderStatus==3 || item.OrderStatus==33 || item.OrderStatus==4) && !item.IsSplitOrder && paramsoff" @click.native="express(item.OrderNo)">查看物流</Button>
                                    <Button class="nopay" v-if="item.OrderStatus==1" @click.native="updateFare(item.OrderNo,item.BuyUserId,index)">修改邮费</Button>
                                    <Button v-if="item.ActivityType!=3 && item.IsHandGift==false && item.OrderStatus==2 && item.IsTransfer==false" @click.native="toCaigou(item.OrderNo,index)">转采购</Button>
                                    <Button v-if="(item.OrderStatus==3 || item.OrderStatus==33 ) && !item.IsSplitOrder && expressage==true"  @click.native="updateCourier(item.OrderNo,index,true)">修改快递</Button>
                                    <Button v-if="(item.OrderStatus==3 || item.OrderStatus==33 ) && item.IsSplitOrder && expressage==true" @click.native="updateCourier(item.SendNo,index,false)">修改快递</Button>
                                </p>
                            </div>
...
export default {
    ...
    methods:{
        ...
        load(bool){//加载数据
            ...
            if (that.selRow.OrderStatus == 3 || that.selRow.OrderStatus == 4) {
                that.shipment = false;
            } else {
                that.shipment = true;
            }
            ...
            wxpost('User/GetSaleOrder',that.selRow).then(res =>{
                that.$Spin.hide();
                if(res.code == 200){
                    console.log(res);
        ...
        goDetails(no){//订单详情
            let that = this;
            that.$router.push({name:'fenxiaoOrderDetails',params:{OrderNo:no}});
        },
        Cancel(no,num){//取消订单
            let that = this;
            that.$Modal.confirm({
                ...
                render: (h) => {
                    return h('div',{class:'model-cont'},[
                        ...
                        h('div',{class:'model-foot','style':{textAlign:'right',marginTop:'0.2667rem'}},[
                            ...
                            h('Button', {'style':{boxShadow:'none',background: '#009788',borderColor:'#009788'},props: {type: 'primary'},on: {click:()=>{
                                if(!Price || Price == ''){
                                    that.$Message.success('请输入金额');
                                    return false;
                                }
                                isLoading(that,'更新邮费中···')
                                wxpost('User/SetPurchaseOrderUpdateFare',{OrderNo:no,BuyUserId:id,FarePrice:Price}).then(res =>{
                                    that.$Spin.hide();
                                    if(res.code == 200){
                                        that.$Message.success(res.message);
                                        that.$Modal.remove();
            ...
            that.$Modal.confirm({
                ...
                render: (h) => {
                    return h('div',{class:'model-cont'},[
                        ...
                        h('div',{class:'model-content'},[
                            h('p', {'style':{fontSize:'0.3733rem'}},'确认该订单转为采购订单吗？')
                        ]),
                        h('div',{class:'model-foot','style':{textAlign:'right',marginTop:'0.2667rem'}},[
                            h('Button', {'style':{boxShadow:'none',borderColor:'#dcdee2',marginRight:'0.1333rem'},on: {click:()=>{that.$Modal.remove();}}},'取消'),
                            h('Button', {'style':{boxShadow:'none',background: '#009788',borderColor:'#009788'},props: {type: 'primary'},on: {click:()=>{
                                ...
                                that.$Modal.remove();
                                wxpost('User/SetTransferSaleOrder',{OrderNo:no}).then(res =>{
                                    that.$Spin.hide();
                                    if(res.code == 200){
        ...
        express(no){//查看物流
            let that = this;
            let url = 'User/GetOrderDeliverByOrderNo/';
        ...
        waitSend(no){//分批发货
            let that = this;
            that.$router.push({name:'fenxiaoWaitSend',params: {OrderNo:no}});//跳转
        },
        send(no){//发货
            let that = this;
            that.$router.push({name:'fenxiaoSend',params: {OrderNo:no}});//跳转
        }
    },
    mounted(){//页面开始加载
        let that = this;
...
<style>
    .fenxiaoOrder .content .title .title_a .ivu-dropdown .ivu-dropdown-rel{
        padding:0.1333rem 0.2667rem;
        background:#DCDDDD;
    }
    .fenxiaoOrder .content .title .title_a .ivu-dropdown .ivu-select-dropdown{
        left:0.4rem;
        width:-moz-calc(100% - 0.8rem);
        width:-webkit-calc(100% - 0.8rem);
        width:calc(100% - 0.8rem);
    }
    .fenxiaoOrder .ivu-dropdown-item-selected,.fenxiaoOrder .ivu-dropdown-item-selected:hover{
        background:none;
        color:#009788;
    }
    .fenxiaoOrder .content .cont>ul>.active a{
        color:#009788 !important;
    }
    .fenxiaoOrder .ivu-btn,.fenxiaoOrder .ivu-btn:hover{
        border-color:#727171;
        color:#727171;
    }
    .fenxiaoOrder .ivu-btn.nopay,.fenxiaoOrder .ivu-btn.nopay:hover{
        border-color:#009788;
        color:#009788;
    }
