<template>
    <div id="container" class="caigouScanRecord">
        <div class="content">
            <div class="title">
            ...
            <div class="cont">
                <ul >
                    <li class="list-cont" v-for="(item,index) in recordData" :key="index">
                        <p><span>条码:</span>{{item.ScanCode}}</p>  
                        <p><span>入库时间:</span> {{item.CreatedTime}}</p>
...
export default {
    ...
    methods:{
        ...
        load(bool){
            ...
            wxpost('User/GetPurchaseScanCode',that.selRow).then(res =>{
                that.$Spin.hide();
                if(res.code == 200){
                    console.log(res);
