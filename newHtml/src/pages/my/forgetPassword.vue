        ...
        <div class="content">
            <div class="cont">
                <Form :model="formItem" :label-width="60" label-position="left">
                    <FormItem label="手机号" >
                        <Input v-model="oldPhone" placeholder="请输入手机号" ></Input>
                    </FormItem>
                    <FormItem label="验证码" >
            ...
            <div class="foot">
                <Button long @click.native="next" >提交</Button>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    methods:{
        ...
        getOldCode(){//获取旧手机号校验码
            ...
            if(!that.oldPhone || that.oldPhone == ''){
                that.$Message.warning('新手机号不能为空');
                return false;
            }
            if(!/^(13[0-9]|14[579]|15[0-3,5-9]|16[6]|17[0135678]|18[0-9]|19[89])\d{8}$/.test(that.oldPhone)){
                that.$Message.warning('手机号格式不对');
                return false;
            }
            that.getCode(that.oldPhone);
        ...
        next(){//下一步
            ...
            wxpost('API/SetForgotPwdModify',{Phone:that.oldPhone,CheckCode:that.formItem.oldCode}).then(res =>{
                if(res.code == 200){
